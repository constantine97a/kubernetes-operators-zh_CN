...
在本章的第一部分，我们概述了运行本书中示例的要求，并就如何建立对满足这些要求的Kubernetes集群的访问提供建议。在第二部分中，您将使用该集群来研究Operator通过安装和使用一个集群所做的工作。到最后，您将拥有一个Kubernetes集群用作Operator测试平台，并且您将知道如何通过一组清单在其上部署现有Operator。
您还将看到Operator在面对更改和故障时管理其应用程序的特定内部状态，从而增加您对后续章节中介绍的Operator架构和构建工具的理解

## 安装 Operator Lab

要在以下章节中构建、测试和运行Operator，您需要集群管理员访问运行Kubernetesv1.11.0或更高版本的集群。如果您已经满足这些要求，则可以跳到下一部分。在本节中，我们向需要搭建Kubernetes集群或需要本地环境进行Operator开发和测试的读者提供一般性建议。
...
<img src="./images/note.png" alt="note" style="zoom:10%;" />[^i]  权限较低的用户可以使用Operator管理的服务和应用程序——“Operand”。
